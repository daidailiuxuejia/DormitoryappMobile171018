 ...
 */
public class ChangeDormActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.change_dorm_view);
		myApp = (MyApp) ChangeDormActivity.this.getApplication();
		...
		buttonSend.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(eventname.equals("") || eventname==null){
					Toast.makeText(ChangeDormActivity.this, "不能为空", Toast.LENGTH_SHORT).show();;
					return ;
				}
//				System.out.println(currenttime);
		...
		imageBack.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				ChangeDormActivity.this.finish();
			}
		});
	}
	...
	public void SendData(String eventname, String eventtime) {
		String url = HttpUtil.URL_CHANGEDORM;
		String query = "";
		query += "xuehao=" + eventname + "&";
		query += "number=" + eventtime ;
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == 200) {
				...
				try {
					...
					if (arrlist != "" && !arrlist.equals("arrlist")
						...
						if (arrlist.equals("1")) {
							Toast.makeText(ChangeDormActivity.this, "恭喜，操作成功",
									Toast.LENGTH_SHORT).show();
							;
							ChangeDormActivity.this.finish();
						} else {
							Toast.makeText(ChangeDormActivity.this, "抱歉，操作失败",
									Toast.LENGTH_SHORT).show();
							;
						}
					}
			...
			e.printStackTrace();
			result = "网络异常！";
			Toast.makeText(ChangeDormActivity.this, result, Toast.LENGTH_SHORT)
					.show();
			;
		} catch (IOException e) {
			e.printStackTrace();
