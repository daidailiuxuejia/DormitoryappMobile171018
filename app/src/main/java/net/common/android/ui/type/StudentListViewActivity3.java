 ...
 */
public class StudentListViewActivity3 extends Activity implements OnScrollListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.student_list3);
		myApp =  (MyApp) StudentListViewActivity3.this.getApplication();
		keyword = StudentListViewActivity3.this.getIntent().getStringExtra("keyword");
		...
		adapter =  new StudentListViewAdapter(StudentListViewActivity3.this);
		goodsListView.setAdapter(adapter);
		goodsListView.setSelection(0);
		String dormitory_no=getIntent().getExtras().getString("dormitory_no");
		
			loadingGoodsListData(dormitory_no);
...
//		loadingGoodsListData(null);
	...
	private void ShowSureDialog(final String roomid) {
		final AlertDialog alertDialog = new AlertDialog.Builder(StudentListViewActivity3.this)
				.setTitle("确认信息")
				.setMessage("确认取消本次预定？")
...
//					JSONObject obj = new JSONObject(json);
					...
					if(arrlist.equals("1")){
						Toast.makeText(StudentListViewActivity3.this, "取消成功", Toast.LENGTH_LONG).show();
//						loadingGoodsListData();
			...
			e.printStackTrace();
			result = "网络异常！";
			Toast.makeText(StudentListViewActivity3.this, result, Toast.LENGTH_SHORT).show();;
		} catch (IOException e) {
			e.printStackTrace();
			result = "网络异常！";
			Toast.makeText(StudentListViewActivity3.this, result, Toast.LENGTH_SHORT).show();;
	...
	public void loadingGoodsListData(String key){
		...
		String url = HttpUtil.URL_STUDENTLST2+""+key;
		HttpPost request = HttpUtil.getHttpPost(url);
		String result = null;
		try {
			// 获得响应对象
